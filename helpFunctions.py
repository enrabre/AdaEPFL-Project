...
def isSpecificEventListIllegal(detectedEventDateList, max_event_duration, min_duration_before_new_event):
    '''
    Detects recurrent events.
    Return true if the list of dates contain illegal tupples (3) of events, so if the event is recurrent
...
def mergeCloseEvents(rowsList, max_event_duration, delimiter):
    '''
    Merge events that are considered as "close" together.
    ...
    def areCloseEvents(event1, event2):
        ...
        '''
        return abs(event1['date'] - event2['date']) < max_event_duration
    ...
    if len(rowsList) < 2:
        ...
        exportedEventList = []
        # This goes through the *sorted* list and add the pair of indices
        # (first indice and last indice) where events that should be merged appear.
...
def to_utc(date):
    '''
    Return the utc time of a date
    :param date: a date in datetime format
    :return: the utc time
    '''
...
def convert_to_unix_time(record):
    '''
    Return the unix time of a year and a month
    :param record: a record of time (with year and month in it)
    :return: the unix time of a defined month and year
    '''
